...
class QAgent(Agent):
    ...
    def get_state(player_position: Tuple[int, int], ghosts_positions: List[Tuple[int, int]]):
        ghosts_positions.append(player_position)
        return tuple(map(tuple, ghosts_positions))
    ...
    def train(self, episodes, **kwargs):
        ...
        stats_filename = "stats_" + self.filename + ".pkl"
        stats = []
...
if __name__ == '__main__':
    run_agent("classic")
    #train_agent(layout="classic", episodes=5000)
    #boost_agent(layout="classic", episodes = 100)
