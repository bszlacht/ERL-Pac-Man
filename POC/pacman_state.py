...
class PacmanState:
    def __init__(self, pacman_pos: Tuple[int, int], ghosts_pos: List[Tuple[int, int]], is_god_mode: int):
        self.pacman_pos = pacman_pos
        self.ghosts_pos = sorted(ghosts_pos, key=lambda t: (t[1], t[0]))
        self.is_god_mode = is_god_mode
    ...
    def get_state(self) -> Tuple:
        return tuple(item for sublist in self.vector  for item in sublist) + (self.is_god_mode,)